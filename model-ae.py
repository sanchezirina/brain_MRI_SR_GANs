...
def autoencoder(x_auto, input_shape=[None, 128, 128, 92, 1], n_filters=[1, 30, 30, 30], filter_sizes=[3, 3, 3, 3]):
    ...
    #x_auto = tf.placeholder(tf.float32, input_shape, name='x_auto')
    current_input = x_auto
    ...
    for layer_i, shape in enumerate(shapes):
        ...
        output = lrelu2(tf.add(
            tf.nn.conv3d_transpose(
                current_input, W,
                tf.stack([tf.shape(x_auto)[0], shape[1], shape[2], shape[3], shape[4]]),
    ...
    cost = tf.reduce_sum(tf.square(y - x_auto))
...
def train(upscaling_factor, residual_blocks, feature_size, path_prediction, checkpoint_dir, img_width, img_height,
    ...
    ae_real_z = tf.placeholder('float32', [int((batch_size * num_patches) / div_patches), 16, 16, 12, 30],
                               name='ae_real_z')
    ae_fake_z = tf.placeholder('float32', [int((batch_size * num_patches) / div_patches), 16, 16, 12, 30],
                               name='ae_fake_z')
